    ...
    >
          ...
          {['company', 'industry', 'financial', 'news'].map((category) => (
            <div key={category} className={`${glassStyle} rounded-xl p-3`}>
              <h3 className="text-base font-medium text-gray-900 mb-3 capitalize">
                {category.charAt(0).toUpperCase() + category.slice(1)} Requêtes
              </h3>
      ...
      {!isExpanded && (
        <div className="mt-2 text-sm text-gray-600">
          {queries.length} Les requêtes générées à travers {['company', 'industry', 'financial', 'news'].length} categories
        </div>
      )}
    </div>
  );
